<?php
	...
	$appstatus_query = mysqli_query("SELECT app_status FROM appointment WHERE app_number='$cancel'",$conn);
	...
	$receiver_query = mysqli_query("SELECT app_doctorusername FROM appointment WHERE app_number='$cancel'",$conn);
	...
	if($appstatus_array[0] == 'Approved') {
		...
		if($receiver_array[0] == $username) {
			$receiver_query = mysqli_query("SELECT app_patientusername FROM appointment WHERE app_number='$cancel'",$conn);
			$receiver_array = mysqli_fetch_array($receiver_query);
		}
		mysqli_close($conn);
	...
	$delete_result = mysqli_query($delete_query,$conn);
